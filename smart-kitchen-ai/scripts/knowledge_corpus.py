"""
C 端 AI 知识库测试语料（菜品知识卡）。

独立成模块以便复用：scripts/generate_test_docs.py 生成 Word/PDF/txt 文件、
eval/run_eval.py 直接以 doc_text() 文本作为 golden 知识入库评测。
本模块不引入 docx/reportlab 等重依赖。

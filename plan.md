...
## 🎯 当前整体进度：Phase 6 Step 5 已完成，Phase 7（优化、测试与Docker部署）待开发
...
- [x] **Step 2: AI 知识库（RAG）**
  ...
  - 实现文档版本管理机制（支持 version/status/effective\_from）。
...
- [x] `/api/order/my-detail/{id}` — 订单详情，含 availableActions（ADD\_DISH/PAY/REVIEW）字段
...
- [x] **小程序项目初始化**：注册小程序 AppID，配置服务器域名白名单（request/uploadFile/socket 合法域名）。
- [x] **网络请求封装**：封装 `wx.request` 为 Promise，统一注入 `Authorization: Bearer <token>`，处理 401 → 清除 token → 重新登录。
- [x] **Token 持久化**：`wx.setStorageSync('token', token)` 存储登录态，小程序启动时读取并校验。
- [x] **SSE 客户端实现**：小程序不支持原生 EventSource，需用 `wx.request` 配合 `enableChunked: true` 实现流式读取（AI 客服）；低版本基础库不兼容时降级为普通 POST 等待完整响应。

### Step 4: C端微信小程序页面 

- [x] **全局布局**：底部 TabBar（菜单 / 订单 / AI客服），`app.json` 配置 tabBar 与页面路由。
- [x] **微信登录**：`wx.login()` 获取 code → `wx.request` 调 `/api/auth/wx-login` → 新用户跳注册页，老用户存 token 进入首页。`app.js` 新增 `checkLogin()` 全局校验方法。
- [x] **用户注册页** `pages/register/index`：昵称/头像/手机号填写 → 调 `/api/auth/register` → 获取 token → 进入首页。
- [x] **菜品浏览** `pages/menu/index`：分类 Tab 筛选 + 菜品卡片（图片/名称/价格/库存状态）+ 加入购物车按钮，购物车角标数量。
- [x] **菜品详情** `pages/dish-detail/index`：点击菜品卡片进入，展示大图/配料/过敏原信息 + 底部加购按钮。
- [x] **购物车状态管理** `utils/cart.js`：使用 `wx.setStorageSync` 持久化，提供 addToCart/updateQuantity/removeFromCart/clearCart/getTotalCount/getTotalAmount 等 7 个方法。
- [x] **选座下单** `pages/order/index`：可视化座位图（从 `/api/seat/available` 获取座位状态，已占用灰色不可选）+ 已选菜品确认列表 + 提交订单（调 `/api/order/submit`）。
- [x] **订单详情** `pages/order-detail/index`：订单状态流展示 + 根据 `availableActions` 动态显示 \[加菜] \[结账] \[评价] 按钮。
- [x] **评价页** `pages/review/index`：五星评分组件 + 文字评价输入框 → 调 `/api/review/submit`。
- [x] **我的订单** `pages/my-orders/index`：历史订单卡片列表（分页加载 + 上拉加载更多），点击跳转订单详情。
- [x] **AI 客服页** `pages/ai-chat/index`：对话列表 + 输入框 + SSE 流式接收 AI 回复（纯文本渲染），包含\[正在输入...]动画提示。

### Step 5: RabbitMQ 订单超时自动取消

**目标**：用户下单后若长时间不支付，订单会永远挂起、已扣库存永久占用。引入 RabbitMQ 死信队列（DLX + TTL）实现「30 分钟内未支付则自动取消并归还库存」。详细设计见 spec.md 第 11 节。

- [x] **RabbitMQConfig**：新增 `config/RabbitMQConfig.java`，声明延迟交换机（`order.delay.exchange`）、延迟队列（TTL 30分钟）、死信交换机（`order.timeout.exchange`）、超时消费队列，以及两对绑定关系。
- [x] **OrderTimeoutConsumer**：新增 `service/OrderTimeoutConsumer.java`，通过 `@RabbitListener` 监听 `order.timeout.queue`，消费时校验订单状态（仅 ORDERED/SERVED 可取消），调用 `orderService.cancelOrder()` 归还库存，已支付/已撤销订单直接 ACK 跳过。
- [x] **OrderServiceImpl 下单埋点**：在 `submitOrder()` 订单落库成功后，调用 `rabbitTemplate.convertAndSend()` 向 `order.delay.exchange` 发送一条延迟消息（消息体仅含 orderId）。
- [x] **单元测试**：验证延迟消息发送与超时消费逻辑，覆盖正常超时取消、已支付跳过、已撤销跳过、订单不存在、消费异常仍ACK等场景。

...
## 🎯 当前整体进度：Phase 1 已完成，准备进入 Phase 2
...
## 🟡 Phase 2: C端核心点单链路 (进行中)
**目标**：顾客从小程序进入，浏览菜品、加入购物车、提交订单、完成模拟结账。
*   [ ] **Dish 模块 (菜品浏览)**
    *   实现按分类查询上架菜品列表接口。
    *   返回数据需包含菜品的基本信息、库存状态及规格配料。
*   [ ] **Order 模块 (下单核心)**
    *   实现提交订单接口 (`/api/order/submit`)。
    *   **重点**：引入 Redis，编写 Lua 脚本实现高并发下的库存安全扣减。
    *   实现订单的落库与状态初始化 (状态变更为 `ORDERED`)。
*   [ ] **Order 模块 (订单流转)**
    *   实现「确认结账」接口 (`/api/order/{id}/pay`)：状态变更为 `PAID`，记录模拟流水号。
    *   实现「撤销订单」接口：退还库存。
...
## ⚪ Phase 4: B端管理与高级功能 (待办)
**目标**：完善系统的后台管理能力，增加项目亮点。
*   [ ] **AdminDish 模块 (菜品管理)**：实现菜品的增删改查、上下架、配料图文上传。
*   [ ] **Stock 模块 (库存管理)**：实现库存盘点、入库/出库流水记录查询。
*   [ ] **Review 模块 (餐后评价)**：实现顾客对 PAID 状态订单的评价提交，以及后台的评价列表查询。
